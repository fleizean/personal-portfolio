...
const ProjectsContent: React.FC = () => {
    ...
    const getStatusBadge = (status: ProjectItem['status']) => {
        switch (status) {
            case 'development':
                return <span className="bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-yellow-900/30 dark:text-yellow-300">In Development</span>;
            case 'completed':
                return <span className="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-green-900/30 dark:text-green-300">Completed</span>;
            case 'ongoing':
                return <span className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-blue-900/30 dark:text-blue-300">Ongoing</span>;
            default:
                return null;
        }
    };
                                    ...
                                    <div className="flex items-baseline gap-2 flex-grow">
                                        ...
                                        <div className="w-full">
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                <span className="font-xs text-gray-800 dark:text-gray-400">• {project.year}</span>
                                                {getStatusBadge(project.status)}
                                            </div>
                                            <p className="text-sm text-black dark:text-gray-400 mt-1">{project.description}</p>

                                            {/* Tech Stack */}
                                            {project.techStack && (
                                                <div className="flex flex-wrap gap-1 mt-2">
                                                    {project.techStack.map((tech, techIndex) => (
                                                        <span key={techIndex} className="bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300 text-xs px-2 py-0.5 rounded">
                                                            {tech}
                                                        </span>
                                                    ))}
                                                </div>
                                            ...
                                            <div className="flex gap-3 mt-2">
                                                {project.githubLink && (
                                                    <Link
                                                        ...
                                                        className="text-sm text-black hover:text-black dark:text-gray-400 dark:hover:text-white"
                                                    >
